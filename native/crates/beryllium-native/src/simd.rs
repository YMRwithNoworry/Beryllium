...
mod avx2_kernels {
    ...
    pub unsafe fn squared_distances_f64_x4(
        ...
        // SAFETY: The caller guarantees four readable packed triples and four writable outputs.
        unsafe {
    ...
    pub unsafe fn intersecting_aabb_f64_x4(
        ...
        // SAFETY: The caller guarantees four readable packed six-value AABBs.
        unsafe {
    ...
    pub unsafe fn potential_energy_partial_x4(
        ...
        // SAFETY: The caller guarantees four readable positions/charges and four writable results.
        unsafe {
...
#[cfg(not(target_arch = "x86_64"))]
pub fn squared_distances_f64_avx2(_: f64, _: f64, _: f64, _: &[f64], _: &mut [f64]) -> usize {
    0
}
...
pub unsafe fn batch_4_distances(
    ...
    // SAFETY: The caller guarantees four packed positions beginning at offset.
    unsafe {
...
#[cfg(not(target_arch = "x86_64"))]
pub fn batch_4_distances(_: f64, _: f64, _: f64, _: &[f64], _: usize, _: &mut [f64; 4]) {}

#[cfg(target_arch = "x86_64")]
#[allow(
    clippy::too_many_arguments,
    reason = "the AVX2 kernel consumes flat AABB bounds"
)]
pub unsafe fn batch_4_aabb_intersections(
    ...
    // SAFETY: The caller guarantees four packed AABBs beginning at offset.
    unsafe {
...
mod tests {
    ...
    fn squared_distances_x4_should_match_scalar() {
        ...
        let positions = [
            0.0, 0.0, 0.0, 3.0, 4.0, 0.0, -1.0, 0.0, 2.0, 100.0, 0.0, -100.0,
        ];
    ...
    fn aabb_intersections_x4_should_exclude_touching_boundaries() {
        ...
        let boxes = [
            ...
            1.0, 0.0, 2.0, 0.0, 1.0, 3.0, 1.0,
        ];

        let mask = unsafe {
            avx2_kernels::intersecting_aabb_f64_x4(0.0, 0.0, 0.0, 1.0, 1.0, 1.0, boxes.as_ptr())
        };
